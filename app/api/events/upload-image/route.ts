...
// POST - Upload image to events bucket
  ...
  try {
    ...
    if (billingStatus.requiresActivation) {
      return NextResponse.json(
        { error: `Activate ${billingStatus.planLabel} in Billing before uploading event assets.` },
        { status: 402 },
      )
    }
    ...
    // Use the actual path from the upload response (more reliable than fileName)
