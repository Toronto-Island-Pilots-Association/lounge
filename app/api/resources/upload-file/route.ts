...
// POST - Upload file to resources bucket
  ...
  try {
    ...
    if (billingStatus.requiresActivation) {
      return NextResponse.json(
        { error: 'Add billing details in Billing before uploading announcement assets.' },
        { status: 402 },
      )
    }
    ...
    if (uploadError) {
      ...
      if (errorMessage.includes('401') || 
        ...
        return NextResponse.json(
          { error: 'Unable to upload file. Please contact support if this issue persists.' },
          { status: 401 }
        )
      }
      return NextResponse.json(
