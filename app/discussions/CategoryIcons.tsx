...
export default function CategoryIcon({ category, className = 'w-4 h-4' }: CategoryIconProps) {
  ...
  switch (category) {
    ...
    case 'flying_at_ytz':
      return (
        <svg className={iconClass} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8M8 11l4-4 4 4" />
        </svg>
      )
    ...
    case 'training_safety_proficiency':
      return (
        <svg className={iconClass} fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
// Large icon component for category pages
export function CategoryIconLarge({ category }: { category: DiscussionCategory }) {
  return <CategoryIcon category={category} className="w-16 h-16" />
}
